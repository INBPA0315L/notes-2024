...
1. Define getter methods for all the fields and setter methods for the modifiable fields as well.
1. Override the inherited `toString()` method to return a well-detailed string representation of the state.
1. Override the inherited `equals()` and `hashCode()` methods to ensure that two instances are equal if and only if their `id` fields and `neptun` codes are the same.
...
1. Each line of the standard output contains a record in the following formats:
    ...
    1. `<age>;<name>;<id>` in the case of a person
    1. `<age>;<name>;<id>;<neptun>;<program>` in the case of a student
...
1. Define the class-level method `query()` in class `Person`! The method gets a `Person` array as its parameter and returns the greatest age that appears inside the array.
1. Define the instance-level method `isAdult()` in class `Person`! The method should return whether the person is an adult (at least `18` years old).
1. Define the instance-level method `isTaxFree()` in class `Person`! The method should return whether the person is tax-free in Hungary (younger than `25` years).
1. Define the `ADULT_LIMIT` named constant in class `Person`, assigning the initial value of `18` to it. Modify method `isAdult()` to use the constant.
1. Define the `TAX_LIMIT` named constant in class `Person`, assigning the initial value of `25` to it. Modify method `isTaxFree()` to use the constant.

### Overloading methods

1. Define the class-level method `query()` in class `Person`! The method gets a `Person` array and a limit as its parameters. The method should return the number of people who are older than the given limit.
1. Try to define the class-level method `query()` in class `Person`! The method gets a `Person` array as its parameter. The method should return the average of the people's ages. What happens?
